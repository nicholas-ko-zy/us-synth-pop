...
pums_hh_indv <- pums %>% 
  mutate(puma = factor(puma),
         ...
         ncars = cut(as.numeric(ncars), breaks=c(-Inf, 0, 1, Inf), labels=c("0", "1", "2+")) %>%
           ordered(),
         nchil = cut(nchil, breaks=c(-Inf, 0, 1, 2, Inf), labels=c("0", "1", "2+", "3+")) %>%
           ordered(),
...
## for SM and NF types, set nwork equal to the number of employed individuals in the household
pums_hh_indv.type$SM$nwork <- as.integer(pums_hh_indv.type$SM$emply %in% c("Civilian employed", "Armed forces employed")) %>%
  ordered(levels = levels(pums_hh_indv.type$MC$nwork))
pums_hh_indv.type$NF$nwork <- pums_hh_indv.type$NF %>% 
  group_by(HHID) %>% 
  summarise(emplyd = sum(emply %in% c("Civilian employed", "Armed forces employed"))) %>% 
  pull(emplyd) %>%
